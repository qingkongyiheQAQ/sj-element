...
describe("Button.vue", () => {
  ...
  it("should has the correct type class when type prop is set", () => {
    ...
    types.forEach((type) => {
      // mount 让你可以： ✅ 渲染 Vue 组件（就像在浏览器里打开一样）
      const wrapper = mount(Button, {
        // 等同于<Button type="primary" />
        props: { type: type as any },
      });
      // **检查这个type是否正确添加到了class` 里**。
      expect(wrapper.classes()).toContain(`sj-button--${type}`);
    });
  });
  ...
  it("should has the correct size class when size prop is set", () => {
    ...
    sizes.forEach((size) => {
      ...
      expect(wrapper.classes()).toContain(`sj-button--${size}`);
    });
  });
...
describe("ButtonGroup.vue", () => {
  ...
  test("button group size", () => {
    ...
    sizes.forEach((size) => {
      ...
      expect(buttonWrapper.classes()).toContain(`sj-button--${size}`);
    });
  });
  ...
  test("button group type", () => {
    ...
    types.forEach((type) => {
      ...
      const buttonWrapper = wrapper.findComponent(Button);
      expect(buttonWrapper.classes()).toContain(`sj-button--${type}`);
    });
  });
